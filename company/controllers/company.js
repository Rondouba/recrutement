...
const getAvgReviewData = async (auth) => {
  ...
  if (!resp) {
    throw Error('pas de réponse du service avis');
  }
  ...
  resp.data.forEach((r) => {
    ...
    if (avgReviewMap[companyId]) {
      ...
      avgReviewMap[companyId].overallRating = (overallRating * totalReviews + r.overallRating) / (totalReviews + 1);
      avgReviewMap[companyId].workLifeBalance = (workLifeBalance * totalReviews + r.workLifeBalance) / (totalReviews + 1);
      avgReviewMap[companyId].compensation = (compensation * totalReviews + r.compensation) / (totalReviews + 1);
      avgReviewMap[companyId].jobSecurity = (jobSecurity * totalReviews + r.jobSecurity) / (totalReviews + 1);
      avgReviewMap[companyId].management = (management * totalReviews + r.management) / (totalReviews + 1);
      avgReviewMap[companyId].jobCulture = (jobCulture * totalReviews + r.jobCulture) / (totalReviews + 1);
...
const getAvgSalaryData = async (auth) => {
  ...
  if (!resp) {
    throw Error('pas de réponse du service utilisateur');
  }
...
const getAllCompanies = async (req, res) => {
  try {
    const {
      all, q, city, byReviewed, byRatings,
    } = req.query;
...
const updateCompany = async (req, res) => {
  try {
    ...
    // vérifier si l'employeur est dans l'entreprise
...
const topTenCeos = async (req, res) => {
  try {
    ...
    // créer une carte de companyId => # de ceoApprovals
    ...
    // obtenir les noms des PDG de toutes ces entreprises
