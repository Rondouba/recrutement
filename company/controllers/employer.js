...
const createEmployer = async (req, res) => {
  try {
    ...
    if (user !== req.body.id) {
      res.status(400).json({
        ...errors.badRequest,
        message: 'employer.id dans le corps doit être identique à celui de l utilisateur connecté',
      });
      return;
    }
...
const updateEmployer = async (req, res) => {
  try {
    ...
    if (user != id) {
      res.status(400).json({
        ...errors.badRequest,
        message: 'l identifiant doit être le même que celui de l utilisateur connecté',
      });
      return;
    }
