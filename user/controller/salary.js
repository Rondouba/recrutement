...
const createSalary = async (req, res) => {
  ...
  if (user !== req.params.id) {
    res.status(400).json({
      ...errors.badRequest,
      message: 'l identifiant dans le chemin doit être le même que celui de l utilisateur connecté',
    });
    return;
  }
  ...
  if (req.body.companyId === null || req.body.companyId === undefined) {
    return res.status(404).send({ status: 400, message: 'Company Id Non trouvee ' });
  }
  ...
  try {
    ...
    if (err.isAxiosError && err.response.status === 404) {
      return res.status(404).send({ status: 400, message: 'Company n existe pas!' });
    }
  }
...
const updateSalary = async (req, res) => {
  ...
  if (req.body.companyId && req.body.companyId !== null) {
    try {
      ...
      console.log(err);
      if (err.isAxiosError && err.response.status === 404) {
        return res.status(404).send({ error: 'Company n existe pas!' });
      }
    }
  }
...
const getSalaryById = async (req, res) => {
  ...
  if (user !== req.params.id) {
    res.status(400).json({
      ...errors.badRequest,
      message: 'user.id dans le corps doit être identique à l utilisateur connecté',
    });
    return;
  }
