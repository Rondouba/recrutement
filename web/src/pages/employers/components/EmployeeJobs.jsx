...
function EmployeeJobs() {
  ...
  const getEmployerDetails = async () => {
    ...
    if (!response) {
      toast.error('Employé introuvable. Veuillez vous connecter en tant qu_employeur !');
      return;
    }
    ...
    <>
      ...
      {jobs ? (
        jobs.length > 0 ? (
          ...
          <p style={{ marginLeft: '25px', marginTop: '20px' }}>Aucune offre publiée pour le moment...</p>
        )
      ) : null}
    </>
  );
