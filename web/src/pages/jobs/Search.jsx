...
function Search({ advancedSearch }) {
  ...
  // Pour obtenir des options pour sélectionner
  const [companyNameOptions, setCompanyNameOptions] = useState([]);

  // Requête - Entrée de recherche pour deux entrées principales
  const [jobFilter, setJobFilter] = useState('');
  const [locationFilter, setLocationFilter] = useState('');

  // Requête - Autres entrées de recherche
  const [datePosted, setDatePosted] = useState('Peu importe la date');
    ...
    >
      ...
      {!advancedSearch ? (
        <span style={{ marginTop: '30px' }}>
          <a href="#">
            Employeurs : publier une offre d'emploi
          </a>
          {' '}
          -votre prochaine embauche est ici
        </span>
      ) : (
        <div
          style={{
