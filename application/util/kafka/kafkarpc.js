...
const TIMEOUT = 8000; // temps de patientce pour la  réponse en ms
let self;
...
function KafkaRPC() {
  ...
  this.response_queue = false; // placeholder pour les futures requetes
  this.producer = this.connection.getProducer();
}

KafkaRPC.prototype.makeRequest = function (topic_name, content, callback) {
  self = this;
  // genere une correlation unique pour tous les appels id
  const correlationId = crypto.randomBytes(16).toString('hex');

  // créer un délai d'attente pour ce qui devrait se passer si nous n'obtenons pas de réponse
  ...
  // crée une entrée de requête à stocker dans un hachage
  const entry = {
    callback,
    timeout: tId, // l'identifiant du délai d'attente afin que nous puissions l'effacer
  };
